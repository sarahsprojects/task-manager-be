  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(`
      CREATE TABLE "task" (
        ...
        CONSTRAINT "FK_task_user" FOREIGN KEY ("userId") REFERENCES "user"("id") ON DELETE SET NULL
      )
    `);

    // Add TaskHistory Table
    await queryRunner.query(`
      CREATE TABLE "task_history" (
        "id" uuid NOT NULL DEFAULT uuid_generate_v4(),
        "taskId" uuid NOT NULL,
        "previousStatus" varchar NOT NULL CHECK ("previousStatus" IN ('pending', 'in-progress', 'completed')),
        "newStatus" varchar NOT NULL CHECK ("newStatus" IN ('pending', 'in-progress', 'completed')),
        "changedAt" TIMESTAMP NOT NULL DEFAULT now(),
        "reason" text NULL,
        CONSTRAINT "PK_94a85bb16d24033a2e17b580b32" PRIMARY KEY ("id"),
        CONSTRAINT "FK_task_history_task" FOREIGN KEY ("taskId") REFERENCES "task"("id") ON DELETE CASCADE
      )
    `);
